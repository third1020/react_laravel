...
detail: detail
      ...
      handleCreate (event) {
        ...
        axios.put(`/api/news/update/${this.props.match.params.id}`, insertdata)
  .then(response => {
    Swal.fire(
        'Successfully',
  ...
  .catch(error => {
    ...
    Swal.fire(
      ...
      componentDidMount(){
        ...
        axios.get('/api/settingnews/index').then(res => {
          this.setState({
        ...
        axios.get(`/api/news/update/${this.props.match.params.id}`).then(res => {
          this.setState({
                ...
                <Col>
                  ...
                  <Col xs="12"  className="form-group">
                    ...
                    <Col md="4" className="form-group">
                      <label htmlFor="feInputState">Setting News</label>
                      <FormSelect
                         id="setting_news_id"
                         name='setting_news_id'
                         className={`form-control ${this.hasErrorFor('setting_news_id') ? 'is-invalid' : ''}`}
                         value={this.state.setting_news_id}
                         onChange={this.handleFieldChange}>
                         <option value="">Choose...</option>

                         {getsetting_news.map((getsetting_news,idx) => (
                            <option key={idx} value={getsetting_news.id}>{getsetting_news.name}</option>
                          ))}
    ...
    export default HocValidateUser(Update_News)
