...
let container;
    ...
    class Update_PersonContact extends Component {
      constructor (props) {
        super(props)
        this.state = {
      ...
      handleCreate (event) {
        ...
        axios.put(`/api/personcontact/update/${this.props.match.params.id}`, insertdata)
  .then(response => {
    Swal.fire(
        'Successfully',
  ...
  .catch(error => {
    ...
    Swal.fire(
      ...
      componentDidMount(){
        ...
        axios.get(`/api/personresponsible/update/${this.props.match.params.id}`).then(res => {
          this.setState({
            company_id   : res.data.user.company_id,
            person_responsible_id   :  res.data.user.person_responsible_id
          })
        }).catch(err => {
          console.log(err);
          this.setState({
    ...
    export default HocValidateUser(Update_PersonContact)
