    ...
    .then(res => {
      ...
      if(this.state.date == "last-month"){
        filterDate = res.data.filter(item => {
           ...
           var todaytime = today.getMonth()+'/'+today.getFullYear()
    ...
    return (
      ...
      <Card small className="h-100">
        <CardHeader className="border-bottom">
          <h6 className="m-0">รายละเอียดคำร้องปัญหาต่างๆ</h6>
        </CardHeader>
        ...
        <center style={{marginBottom:'20px',marginTop:'50px'}}><h5 style={{color:'#0088FE'}}>Pending {this.state.data[0].value}</h5>
                                              <h5 style={{color:'#00C49F'}}>Approve {this.state.data[1].value}</h5>
                                              <h5 style={{color:'#FFBB28'}}>Auditor {this.state.data[2].value}</h5>
                                              <h5 style={{color:'#FF8042'}}>Rejected {this.state.data[3].value}</h5></center>
