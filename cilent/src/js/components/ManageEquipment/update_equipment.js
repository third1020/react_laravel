...
let container;
    ...
    class Update_equipment extends Component {
      ...
      handleCreate (event) {
        ...
        axios.put(`/api/equipment/update/${this.props.match.params.id}`, insertdata)
  .then(response => {
    Swal.fire(
        'Successfully',
  ...
  .catch(error => {
    ...
    Swal.fire(
      ...
      componentDidMount(){
        axios.get(`/api/equipment/update/${this.props.match.params.id}`).then(res => {
          this.setState({
    ...
    export default HocValidateUser(Update_equipment)
