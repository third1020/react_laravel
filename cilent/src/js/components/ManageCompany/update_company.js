...
class Update_company extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    handleCreate(event) {
        ...
        axios
            .put(
                `/api/company/update/${this.props.match.params.id}`,
                insertdata
            )
            .then(response => {
    ...
    componentDidMount() {
        axios
            .get(`/api/company/update/${this.props.match.params.id}`)
            .then(res => {
                this.setState({
...
export default HocValidateUser(Update_company);
