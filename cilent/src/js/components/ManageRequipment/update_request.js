...
let container;
    ...
    this.setState({[event.target.name]: event.target.value});
      ...
      handleCreate (event) {
        ...
        axios.put(`/api/request_update/${this.props.id}`, insertdata)
          .then(response => {
      ...
      componentWillMount () {
        ...
        axios.get(`/api/request/${this.props.id}`).then(response => {
          this.setState({

            request_tital:  response.data[0].request_tital,
            request_detail:  response.data[0].request_detail,
